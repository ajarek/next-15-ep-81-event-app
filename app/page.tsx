...
export default async function Home({searchParams,}: {searchParams: Promise<{event: string, location: string, name: string, time: string  }>}) {
  ...
  return (
    ...
    <CardHeader className='relative'>
      <Image src={item.img_sm} alt={item.title} width={300} height={300} className='rounded-xl'/>
      <div className='absolute bottom-0 w-20 py-2 px-4 bg-primary text-white text-center rounded-xl'>{item.type}</div>
    </CardHeader>
    <CardContent>
